...
import { Check, Sparkles, Loader2, Wand2, Type, FileEdit, Zap, GripVertical, Plus, Trash2, Copy, ChevronDown, ChevronUp } from "lucide-react";
...
function PremiumDynamicList({ items = [], onChange, fields, addButtonLabel, emptyMessage, renderExtra }: any) {
          ...
          <AnimatePresence>
            {items.map((item: any, i: number) => (
              ...
              >
                ...
                {renderExtra && renderExtra(i, item)}
              </PremiumSectionCard>
            ))}
          </AnimatePresence>
        ...
        <ResumeFormStep title="Work Experience" description="Highlight your professional roles and impact." stepNumber={4} totalSteps={TOTAL_STEPS}>
          <Controller name="experience" control={control} render={({ field }) => (
            <PremiumDynamicList
              ...
              renderExtra={rewriteCallbacks ? (index: number, item: any) => {
